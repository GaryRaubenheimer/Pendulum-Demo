...
class gui_Sidebar(Gui):
    ...
    def change_GuiToMenu(self):
        self.state = "MENU"
        self.kill_gui_widget_list()
        constants.change_state("STARTMENU")
        # self = changeGui("MENU")
...
class gui_createPendulum(gui_Sidebar):
    ...
    def create_createPendulum_widget_list(self):
        gui_widget_list = {
            "labels": self.create_createPendulum_label_dict(
                x_offset, y_offset, slot_height
            ),
            "buttons": self.create_createPendulum_button_dict(
                x_offset, y_offset, slot_height
            ),
    ...
    def create_createPendulum_slider_dict(self, x_offset, y_offset, slot_height):
        # (self, x, y, width, height, min_value=0.0, real_value=0.5, max_value=1.0, color=BLUE, action=None)
        slider_dict = {
            "slider_RGB_RedTrace": Slider(
                x_offset + 5, slot_height * 10, 250, 10, 0, 0, 255, action=self.setRed
            ),
            "slider_RGB_BlueTrace": Slider(
                x_offset + 5, slot_height * 12, 250, 10, 0, 0, 255, action=self.setBlue
            ),
            "slider_RGB_GreenTrace": Slider(
                x_offset + 5, slot_height * 14, 250, 10, 0, 0, 255, action=self.setGreen
            ),
...
class gui_editPendulum(gui_Sidebar):
    ...
    def create_pen_edit_widget_list(self):
        gui_widget_list = {
            ...
            "toggleButtons": self.create_guiEdit_toggleButton_dict(
                x_offset, y_offset, slot_height
            ),
            "radioButtons": self.create_guiEdit_radioButton_dict(
                x_offset, y_offset, slot_height
            ),
    ...
    def create_guiEdit_label_dict(self, x_offset, y_offset, slot_height):
        ...
        label_dict = {
            ...
            "label_Rod_Number": Label(
                x_offset, y_offset + slot_height * 1, "Rod Selection:"
            ),
            "label_Pin_Friction": Label(
                x_offset, y_offset + slot_height * 3, "Pin Friction:"
            ),
            "label_Rod_Length": Label(
                x_offset, y_offset + slot_height * 5, "Rod Length:"
            ),
            "label_Bob_Weight": Label(
                x_offset, y_offset + slot_height * 7, "Bob Weight:"
            ),
            "label_Bob_Radius": Label(
                x_offset, y_offset + slot_height * 9, "Bob Radius:"
            ),
            "label_Trace_Points": Label(
                x_offset, y_offset + slot_height * 11, "Trace Points On/Off:"
            ),
            "label_TP_Off": Label(
                x_offset + 15, y_offset + slot_height * 12 - 10, "Off"
            ),
            "label_TP_Dot": Label(
                x_offset + 75, y_offset + slot_height * 12 - 10, "Dot"
            ),
            "label_TP_Line": Label(
                x_offset + 165, y_offset + slot_height * 12 - 10, "Line"
            ),
        }
        return label_dict
                                ...
                                for cat, wids in self.gui_widget_list.items():
                                    if cat == "sliders":
                                        for name, widget in wids.items():
                                            self.change_guiEdit_widget_slider_info(
                                                name, widget, rod_info
                                            )
    ...
    def check_trace_point_state(self):
        for rod in self.selected_pendulum.rods:
            if (
                ...
                and rod.pin_2.trace_points_isOn == False
            ):
                ...
                and rod.pin_2.trace_points_isOn == True
            ):
                if self.gui_widget_list:
                    for category, widgets in self.gui_widget_list.items():
                        for name, widget in widgets.items():
...
class gui_startMenu(Gui):
    ...
    def select_start(self):
        constants.change_state("SIMULATION")

    def select_about(self):
        constants.change_state("ABOUTMENU")

    def select_quit(self):
        constants.change_state("QUIT")
