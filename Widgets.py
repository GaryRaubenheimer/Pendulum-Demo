...
class RadioButton:
    ...
    def draw(self, screen):
        ...
        if self.is_checked:
            pygame.draw.circle(
                screen, self.check_color, (self.x, self.y), self.radius // 2
            )
...
class Slider:
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(
            screen, self.color, (slider_button_x, self.y), self.slider_button_radius
        )
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            slider_button_x = int(self.x + self.width * self.value)
            if (
                slider_button_x - self.slider_button_radius
                <= mouse_x
                <= slider_button_x + self.slider_button_radius
                and self.y - self.slider_button_radius
                <= mouse_y
                <= self.y + self.slider_button_radius
            ):
                self.dragging = True
