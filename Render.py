        ...
        if not rod.pin_2.trace_points_isLine:
            ...
            for i in range(len(rod.pin_2.trace_points)):
                if i > 0:
                    pygame.draw.line(
                        screen,
                        rod.pin_2.trace_points_colour,
                        rod.pin_2.trace_points[i - 1],
                        rod.pin_2.trace_points[i],
                    )


def draw_bar(screen, rod):
    pygame.draw.line(
        screen, rod.bar.colour, rod.pin_1.position, rod.pin_2.position, rod.bar.width
    )
...
def draw_pins(screen, rod):
    if rod.rod_id == 1:
        pygame.draw.circle(
            screen, rod.pin_1.colour, rod.pin_1.position, rod.pin_1.radius
        )
