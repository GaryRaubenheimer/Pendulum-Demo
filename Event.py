...
def update_held_pendulum(M, pos):
    if M.collision_item:
        ...
        if held_pin == held_pendulum.rods[0].pin_1:
            ...
            held_pin == held_pendulum.rods[0].pin_2
            and held_pendulum.type == constants.DOUBLE
        ):
...
def update_first_pin(pos, held_pendulum):
    temp_pos = pos.copy()
    temp_pos[0] = max(
        constants.BORDER_THICKNESS + 2.5,
        min(pos[0], constants.WIDTH / 4 * 3 - constants.BORDER_THICKNESS - 2.5),
    )
    temp_pos[1] = max(
        constants.BORDER_THICKNESS + 2.5,
        min(pos[1], constants.HEIGHT - constants.BORDER_THICKNESS - 2.5),
    )
...
def apply_mouse_velocity_to_rod(M, dt, rod):
    ...
    if mouse_velocity_angle != 0:
        beta = mouse_velocity_angle - (
            (3 * math.pi - rod.angular_position) % (2 * math.pi)
        )
